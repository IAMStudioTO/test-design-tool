  ...
  return (
    <div style={{ width, height, background: palette?.background || "#0b0f19", color: palette?.headline || "#fff", position: "relative", overflow: "hidden" }}>
      ...
      <div style={{ padding: 64 }}>
        ...
        <h1 style={{ ...H, margin: "120px 0 0 0", fontSize: Math.round(width * 0.076), lineHeight: 1.02 }}>
          {content?.headline}
        </h1>

        <p style={{ ...S, margin: "14px 0 0 0", fontSize: Math.round(width * 0.025), color: palette?.subheadline || "#e5e7eb" }}>
          {content?.subheadline}
        </p>

        <div style={{ ...S, marginTop: 22, fontSize: Math.round(width * 0.018), lineHeight: 1.4, color: palette?.subheadline || "#cbd5e1", maxWidth: Math.round(width * 0.72) }}>
          {content?.body}
        </div>
